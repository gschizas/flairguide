...
def createimages():
    ...
    for i in unique(displayteams.File):
        ...
        if (numpy.min(fileteams.File) == "Images"):
            folder = "full60s"
        else:
            folder = "full60"
        flairlocs = []
        ...
        for j in range(len(flairlocs)):
            fileimage.paste(Image.open(folder + "/" + flairlocs[j][0] + ".png"),
                            (int(flairlocs[j][2]), int(flairlocs[j][1])))
            ...
            if (row != 0):
                ...
                rown = "0"
            if (flairlocs[j][2] != 0):
                coln = "-" + str(int(flairlocs[j][2] * scalefactor)) + "px"
            else:
                coln = "0"
            flaircoords = []
            teamrow = allteams[allteams.Filename == flairlocs[j][0]]
            teamindex = map(str, allteams.Filename).index(flairlocs[j][0])
            if str(teamrow.Shortcutflair[teamindex]) != "None": flaircoords.append(
                '.flair-' + str(teamrow.Shortcutflair[teamindex]) + ':before')
            if str(teamrow.Shortcutinline[teamindex]) != "None": flaircoords.append(
                'a[href="' + str(teamrow.Shortcutinline[teamindex]) + '"]:before')
            if str(teamrow.Shortcutletter[teamindex]) != "None": flaircoords.append(
                'a[href="' + str(teamrow.Shortcutletter[teamindex]) + '"]:before')
            if len(flaircoords) > 0: flairw.write(
                ','.join(flaircoords) + "{background-position:" + coln + " " + rown + "}")
    ...
    for i in unique(flairteams.Division):
        ...
        if len(fileteams) > 0:
            ...
            for j in unique(fileteams.Conf):
                ...
                if len(confteams) > 0:
                    wikitext.write("\n## " + numpy.min(
                        confteams.Conference) + "\n\n|||||||||\n|:--|:--|:--|:--|:--|:--|:--|:--|\n")
                    conflist = []
                    for k in confteams.Filename:
                        teamrow = allteams[allteams.Filename == k]
                        teamindex = map(str, allteams.Filename).index(k)
                        nametourl = str(teamrow.Name[teamindex]).replace("(", "%28").replace(")", "%29").replace("&",
                                                                                                                 "%26")
                        if str(teamrow.Shortcutinline[teamindex]) != "None":
                            teaminline = "[" + str(teamrow.Abbreviated[teamindex]) + "](" + str(
                                teamrow.Shortcutinline[teamindex]) + ")"
                        else:
                            teaminline = "[" + str(teamrow.Abbreviated[teamindex]) + "](#f/" + str(
                                teamrow.Flair1[teamindex]) + ")"
                        teamfull = teaminline + "|[" + str(teamrow.Name[
                                                               teamindex]) + "](/message/compose/?to=cfbflair&subject=flair&message=" + nametourl + ")"
                    ...
                    spotsorder = sorted(range(len(spotsusedy)),
                                        key=lambda lambdakey: spotsusedy[spotsxorder][lambdakey])
            ...
            for j in unique(fileteams.Conf):
                ...
                if len(confteams) > 0:
                    ...
                    for k in confteams.Filename:
                        ...
                        teamindex = map(str, allteams.Filename).index(k)
                        if str(teamrow.Shortcutinline[teamindex]) != "None":
                            wikiinline.write("|" + str(teamrow.Name[teamindex]) + "|\[" + str(
                                teamrow.Abbreviated[teamindex]) + "](" + str(
                                teamrow.Shortcutinline[teamindex]) + ")|[" + str(
                                teamrow.Abbreviated[teamindex]) + "](" + str(
                                teamrow.Shortcutinline[teamindex]) + ")|\n")
                        else:
                            wikiinline.write("|" + str(teamrow.Name[teamindex]) + "|\[" + str(
                                teamrow.Abbreviated[teamindex]) + "](#f/" + str(
                                teamrow.Flair1[teamindex]) + ")|[" + str(
                                teamrow.Abbreviated[teamindex]) + "](#f/" + str(teamrow.Flair1[teamindex]) + ")|\n")
    wikiinline.write("\n\n*Created by /u/bakonydraco, available at /r/flairguide/wiki.*")
    wikiinline.close()
    ...
    try:
        ...
        if (os.path.isfile(newimage)):
            ...
            if (os.path.isfile(oldimage)):
                if Image.open(newimage).size != Image.open(oldimage).size or not ImageChops.difference(
                        Image.open(newimage), Image.open(oldimage)).getbbox() is None:
                    print (
                    str(subreddit) + " " + str(image) + " update time: " + strftime("%Y-%m-%d %H:%M:%S", gmtime()))
        ...
        print "Error Replacing Image" + image + subreddit
...
def update():
    ...
    if hardupdate == True or filechanged:
        ...
        try:
            ...
            print("Error Setting Stylesheet")
...
def updatewikis():
    ...
    try:
        ...
        print "Error Updating Flair Text"
    ...
    try:
        ...
        print "Error Updating Inline Flair"
