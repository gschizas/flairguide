...
for filename in os.listdir("flairedit"):
    ...
    os.system("convert flairedit/" + filename + " -resize '" + str(filewidth) + "x" + str(
        fileheight) + "' -bordercolor none -border " + str(border) + " -gravity center -crop " + str(
        filewidth) + "x" + str(fileheight) + "+0+0! flairedit/" + filename)
    os.system("convert flairedit/" + filename + " -filter RobidouxSharp -resize " + str(width) + "x" + str(
        height) + "! -define png:format=png32 flairedit/" + filename)
