...
protectusers = ["nolez", "diagonalfish", "Honestly_", "sirgippy", "thrav", "bakonydraco", "Corporal_Hicks",
                "blueboybob", "CoachHerbHand", "KKTTU", "rsmith26", "TheBobbyBowden", "FauxPelini", "srs_house",
                "Alpha_Orange", "ttsci", "StrawberryTea", "CoachBrohm"]
...
for item in sub.get_flair_list(limit=None):
    ...
    if css_class == "None" or css_class == "":
        ...
        if (len(csslist) == 8):
            ...
            newcss = allteams.Flair1[map(str, allteams.Filename).index(flaira)] + '-' + allteams.Flair2[
                map(str, allteams.Filename).index(flairb)]
            if user in protectusers:
                newflair = flair_text
            else:
                newflair = allteams.Flairtext[map(str, allteams.Filename).index(flaira)] + ' / ' + allteams.Flairtext[
                    map(str, allteams.Filename).index(flairb)]
            ...
            if user in protectusers:
                ...
                newflair = allteams.Flairtext[map(str, allteams.Filename).index(flair)]
        if newflair != flair_text or newcss != css_class:
            try:
                print(
                "Modifying user " + user + " from " + css_class + " to " + newcss + " and " + flair_text + " to " + newflair)
