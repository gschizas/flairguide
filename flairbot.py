...
def checkmessages(j, r):
    for msg in r.get_unread(limit=None):
        ...
        if (subj == "flair"):
            setflair(auth, body, r)
        elif (subj == "flairuser"):
            setflairuser(auth, body, r)
        elif (subj == "award"):
            logaward(auth, body, msg, r)
        elif (subj == "grantaward"):
            grantaward(auth, body, msg, r)
        else:
            r.send_message(auth, 'Command not recognized',
                           "Sorry, the subject line of your message is not a command I recognize." + robotmessage)
        msg.mark_as_read()
    return (j)
...
def setflairuser(auth, body, r):
    if auth in modlist["cfb"]:
        ...
        r.send_message(auth, 'Permission Denied',
                       "Sorry, you don't have permission to set images for that subreddit." + robotmessage)
...
def settingflair(auth, teamname, teamflair, r):
    try:
        ...
        message = 'Setting Flair: ' + auth + ' : ' + teamname + ' @ ' + strftime("%Y-%m-%d %H:%M:%S",
                                                                                 gmtime()) + " to " + subreddit
        ...
        if checkteam(teama1):
            if checkteam(teamb1):
                if not checkaward(auth, allteams.Filename[map(str, allteams.Name).index(teama)], r):
                    r.send_message(auth, "You haven't earned that!",
                                   "You haven't earned that award yet." + robotmessage)
                    ...
                    if not checkaward(auth, allteams.Filename[map(str, allteams.Name).index(teamb)], r):
                        ...
                        if (teama == teamb):
                            r.send_message(auth, "Two of the same",
                                           "At this point we do not allow you to select two flairs from the same school, both for stylistic and flair count reasons." + robotmessage)
                            team = teama
                            retcss = allteams.Flair[map(str, allteams.Name).index(teama1)]
                        else:
                            retcss = allteams.Flair1[map(str, allteams.Name).index(teama1)] + "-" + allteams.Flair2[
                                map(str, allteams.Name).index(teamb1)]
    else:
        team1 = team.strip()
        try:
            ...
            if checkteam(team.strip()):
                if not checkaward(auth, allteams.Filename[map(str, allteams.Name).index(team1)], r):
                    ...
                    retcss = allteams.Flair[map(str, allteams.Name).index(team1)]
        except:
            retcss = ""
    if retcss != "":
        settingflair(auth, team, retcss, r)
    else:
        r.send_message(auth, 'Team not found',
                       "It looks like you were trying to set your flair for a team we don't have." + robotmessage)
...
def grantaward(auth, body, msg, r):
    if auth in modlist["cfb"]:
        try:
            ...
            tables[tablenum] = tables[tablenum].rstrip() + "\n|/u/" + authuser.replace("/u/",
                                                                                       "") + "|[" + verificationtitle + "](" + verificationlink + ")|\n\n"
            awardswikiupdate = "##".join(tables)
            sub = r.get_subreddit(allsubreddits[0])
            sub.edit_wiki_page("awards", awardswikiupdate, reason='')
            time.sleep(2)
            r.send_message(authuser, 'You won a ' + bodyuser + ' award flair!',
                           "Check out our [Awards Page](/r/cfb/wiki/awards) for details on how you won.  You can now change to and from it at any time from the [flair selector](/r/cfb/wiki/flair)." + robotmessage)
        except:
            print "Error granting award"
    else:
        r.send_message(auth, 'Permission Denied',
                       "Sorry, you don't have permission to set images for that subreddit." + robotmessage)
        print "Permission Denied: " + subreddits
